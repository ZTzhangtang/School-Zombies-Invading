/* ============================================================
 * mp-bridge.js v3 — 地图页联机桥（含队友血条 + 丧尸同步桥接）
 * 零侵入版：钩住 renderer.render 抓取 scene/camera，自带 rAF 心跳
 * 地图页引入顺序：three.js → p2p.js → mp-bridge.js → zsync.js
 * ============================================================ */
window.MP = (() => {
  'use strict';
  ...
  const avatars = new Map();   // id → {group, tgt, scene, hpBar}
  const tracers = [];
  ...
  function tagSprite(text){
    ...
    roundRect(c, 4, 6, 248, 52, 14); c.fill();
    c.fillStyle = '#9fe8c1';
  ...
  function syncAvatars(){
    ...
    P2P.eachRemote(s => {
      ...
      /* ✅ 新增：更新血条 */
      if (a.group.userData.hpBar && s.hp != null){
        drawHpBar(a.group.userData.hpBar, s.hp / 100);
        a.group.userData.hpBar.visible = s.hp > 0;
      }
    });
    avatars.forEach((a, id) => {
      if (!seen.has(id) || a.scene !== scene){
        if(a.scene===scene) scene.remove(a.group);
        avatars.delete(id);
      }
    });
  ...
  async function boot(){
    ...
    catch (e){ badge('❌ 联机失败: ' + (e.type || e.message || e)); return; }

    P2P.on('shot', d => { if (d && d.s) spawnTracer(d.s.x, d.s.y || 1.6, d.s.z, d.s.ry || 0); });
    P2P.on('host-lost', () => badge('⚠ 与房主断线'));

    /* ✅ 新增：把丧尸快照/全局状态桥接到地图页的钩子 */
    P2P.on('zombies',     snap => { if (window.__zombieSync) try { window.__zombieSync(snap); } catch (_) {} });
  ...
  return {
    tick: (cam, scn) => { if (cam) camera = cam; if (scn) scene = scn; },
    setHP: v => { hp = v; },   // 地图页在本地 HP 变化时调用
    state
  };
})();
